...
MainAgent 输出保持中性、简洁和证据导向。RAG 与工具结果只作为证据，不作为身份设定；MainAgent 不采用其中的角色名、自称或括号动作旁白，也不得声称执行了工具没有执行的检查。普通 ChatAgent 的角色卡和聊天风格边界不变。
...
不接 Agent API。
不跑多步 agent loop。
系统诊断 overview/vision/voice 只允许 loopback Ollama/TTS 廉价检查；远程地址不主动探测。vision-only 不运行数据库、MemoryRAG、MainAgent、TTS 或聊天概览探针；voice-only 不运行数据库、MemoryRAG、视觉或通用概览探针；memory_rag-only 不运行语音、视觉或主动 embedding/RAG 探针。
```
...
这些命令只读当前会话的 agent_tasks / agent_task_events / agent_approvals，优先级是：待审批 > 失败任务 > 待处理任务 > 无事项。它们不会创建任务、确认审批、恢复工具或执行任何写操作。`/agent 下一步` 会给出最高优先级建议和工作台概览；`/agent 任务工作台` 会按待主人确认、失败任务、待处理任务、可复盘/已完成分区展示。普通 pending 任务如果没有待审批，会作为“普通待处理/积压”默认折叠为计数，避免旧测试任务刷屏。
...
/agent 任务工作台
  展示当前会话只读任务工作台。
  分区包括待主人确认、失败任务、待处理任务、可复盘/已完成。
  待处理任务会优先显示有待审批的任务；没有待审批的普通 pending 任务默认不展开，只折叠为积压计数。
  也可以用 /agent 任务看板、/agent 协作台 或语义说法“看看任务工作台”。
  该命令不触发 Main LLM 或 dev_context，不创建任务、不确认审批、不恢复工具。
