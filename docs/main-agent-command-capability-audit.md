...
| `reliability_trend` / “查看故障趋势” | `agent_reliability_trend_reply`；SQLite `mode=ro` 读取 P2.47 五分钟结构化事件桶，聚合 24 小时详情和 7 天摘要 | 确定性分类先于 A-LLM / 无 RAG | 显示 component/operation、类别/code、次数、首末时间与恢复状态；不显示 runtime UUID、数据库 ID 或原始异常。无故障不等于证明持续在线 | QQ live、Owner Console GET-only 页、生产 API 和人工浏览器验收已通过；Owner Console 显示 7 项范围。媒体 live 已写入 vision 失败、TTS 冷启动超时与后续成功，TTS 故障组具备恢复证据 |
...
| `vision_status` / “查看视觉状态、识图、Ollama” | `DiagnosticsGraph/VISION -> format_vision_status`；配置、图片缓存、Ollama 服务/模型及当前实现的推理自检 | A-LLM 可选 / 无 | 快速状态；只有服务/模型正常且自检明确返回低质量重复内容时，中性提示 runner 可能异常，并给出手动 `ollama stop <model>`；本次不执行、不重试 | formatter 正常、服务失败、低质量专属建议和其他失败不误提示均有测试；状态命令 live 已验证。真实用户图片先记录 invalid_model_response，补丁加载后又成功识别并形成 recovered；整批失败固定提示已通过自动化，但本次成功路径未覆盖 QQ 失败文案 live |
...
| `tts_status` / “语音状态、TTS 是否在线、IndexTTS2 加载了吗” | `DiagnosticsGraph/TTS -> tts_health_snapshot -> tts_status_reply_lines`；仅主动访问 loopback，读取最近候选 | A-LLM 可选 / 无 | 快速状态；展示按需待机、health、loaded、language，并区分 Web 服务在线与模型显存驻留。实用 | 对象+状态分类正反例、报告 evaluator 和状态 live 覆盖；真实 TTS 在两次冷启动超时后成功合成，主人确认 QQ 语音送达，端到端 live 已通过 |
...
2. 为 28 个 `OWNER_READ_COMMANDS` 建立参数化的“表达 -> command -> provider”矩阵，而不是只覆盖代表性批次。
